...
const datatieto = async () => {
  try {
    ...
    data.forEach((item) => {
      ...
      lisää.innerHTML += `
          <tr>
        <th colspan=3>${item.day}</th>
      </tr>`;

      item.annokset.forEach((annos) => {
        const annoksetTaulukko = `
                <tr>
                  <td>${annos.nimi}<br>
                  ${annos.allergeenit}</td>
                  <td>${annos.hinta}</td>
                  <td><button id="annos-${annos.annos_id}" class="add-btn">+</button></td>
                </tr>
                `;

        // Lisätään lista päivän alle
        lisää.insertAdjacentHTML("beforeend", annoksetTaulukko);
      });
    });
  } catch (error) {
